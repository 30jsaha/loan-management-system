...
class FrontEndController extends Controller
{
    public function sendLoanMail (Request $request)
    {
        ...
        try {
            ...
            $adminEmail = config('mail.from.address') ?? env('MAIL_ADMIN_ADDRESS', 'jsaha.adzguru@gmail.com');
            ...
            Mail::raw($body, function ($message) use ($validated, $adminEmail) {
                $message->to($adminEmail)
                        ->subject('Website Loan Inquiry — ' . ($validated['name'] ?? 'Unknown'));
            ...
            return response()->json([
                'message' => '✅ Your inquiry was sent successfully. We will contact you shortly.'
            ], 200);
        } catch (\Exception $e) {
    ...
    public function sendContactMail (Request $request)
    {
        ...
        try {
            ...
            $bodyLines = [
                ...
                "Service: " . ($validated['service'] ?? '-'),
                "",
                "This message was generated by the frontend contact enquiry system."
            ];
            $body = implode("\n", $bodyLines);

            Mail::raw($body, function ($message) use ($validated, $adminEmail) {
                $message->to($adminEmail)
                        ->subject('Website Contact Inquiry — ' . ($validated['name'] ?? 'Unknown'));
                if (!empty($validated['email'])) {
                    $message->replyTo($validated['email'], $validated['name'] ?? null);
                }
            });
            ...
            Log::error('send_contact_mail error: ' . $e->getMessage());
