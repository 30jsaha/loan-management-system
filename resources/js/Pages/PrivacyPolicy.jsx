        ...
        <div className="container">
          ...
          <div className={`navbar-collapse ${isDesktop ? "show" : isNavOpen ? "show" : "collapse"}`} id="navbarNav">
            <ul className="navbar-nav mx-auto">
              ...
              <li className="nav-item"><Link className="nav-link" href={`${route("homes")}#contact`}>Contact</Link></li>
            </ul>

            {/* Buttons */}
            <div className="d-flex align-items-center gap-2">
              <button className="btn" style={{ backgroundColor: "#008037", color: "white" }}>
                Download Brochure
              </button>

              {auth?.user ? (
                <Link href={route("dashboard")}>
                  <button className="btn" style={{ backgroundColor: "#D71920", color: "white" }}>Dashboard</button>
                </Link>
              ) : (
                <Link href={route("login")}>
                  <button className="btn" style={{ backgroundColor: "#D71920", color: "white" }}>Login</button>
                </Link>
              )}
            </div>
