...
import { Pencil, Trash2, Search, ArrowUpDown, ArrowLeft, Repeat } from "lucide-react";
import { Modal, Button, Row, Col, Badge } from "react-bootstrap";

export default function OngoingLoans({ auth, approved_loans }) {
    ...
    // const percentOfPaidAmount = useMemo(() => {
    //     const totalRepayAmount = filteredLoans.reduce((sum, loan) => {
    //         return sum + (parseFloat(loan.total_repay_amt || 0));
    //     }, 0);

    //     if (totalRepayAmount === 0) return 0;
    //     return (totalPaidFiltered / totalRepayAmount) * 100;
    // }, [filteredLoans]);
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800">Ongoing Loans</h2>}
        >
            <Head title="Ongoing Loans" />
                            ...
                            <table className="min-w-full border-collapse border text-sm rounded-lg overflow-hidden shadow-sm">
                                <thead className="bg-green-700 text-white text-xs">
                                    <tr>
                                        ...
                                        <th className="p-2 border"
                                        onClick={() => handleSort("total_percent_paid")}
                                        >Total Paid Percent
                                         <SortIcon column="total_repay_percent" />
                                        </th>
                                        <th className="p-2 border cursor-pointer" onClick={() => handleSort("disbursement_date")}>
                                            EMI Start Date <SortIcon column="disbursement_date" />
                                        </th>
                                ...
                                <tbody>
                                    {paginatedLoans.map((loan) => {
                                        ...
                                        const firstPayment = loan.installments
                                            ?.filter((i) => i.status === "Paid")
                                            .sort((a, b) => new Date(a.payment_date) - new Date(b.payment_date))[0];

                                        const percentOfPaidAmount = loan.total_repay_amt ? (paidAmount / Number(loan.total_repay_amt) * 100) : 0;

                                        return (
                                            <tr key={loan.id} className="border hover:bg-gray-50 transition">
                                                <td className="p-2 border text-sky-600 hover:text-sky-700 hover:underline font-medium">
                                                    <HrefLink
                                                ...
                                                <td className="p-2 border text-green-700 font-bold">
                                                    {percentOfPaidAmount.toFixed(2)}%
                                                </td>
                                                <td className="p-2 border">{firstPayment?.payment_date || "N/A"}</td>
                                                <td className="p-2 border text-center">
                                                    <button
                                                        onClick={() => handleViewDetails(loan)}
                                                        className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-xs shadow-sm"
                                                    >
                                                        <Eye size={14} /> <span>View</span>
                                                    </button>
                                                    <button
                                                        onClick={() => handleRefinancing(loan)}
                                                        disabled={percentOfPaidAmount < 80}
                                                        className={
                                                            "inline-flex items-center gap-2 px-3 py-1 rounded text-xs shadow-sm ml-2 " +
                                                            (percentOfPaidAmount < 80
                                                                ? "bg-green-700 opacity-50 cursor-not-allowed text-white"
                                                                : "bg-green-700 hover:bg-green-800 text-white")
                                                        }
                                                    >
                                                        <Repeat size={14} /> <span>Refinance</span>
                                                    </button>
                                                </td>
                                            </tr>
                                        );
